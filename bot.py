...
@bot.event
async def on_ready(): # memanggil fungsi saat bot sudah siap
    print(f'Bot is ready. Logged in as {bot.user}')

@bot.command(name='start')
async def start_command(ctx): # memanggil fungsi saat perintah !start
    await ctx.send("Halo! Saya adalah bot manajer proyek\nSaya akan membantu kamu menyimpan proyek dan informasi tentangnya!)")
    await info(ctx)

@bot.command(name='info')
async def info(ctx): # memanggil fungsi saat perintah !info untuk menampilkan informasi tentang bot
    await ctx.send("""
...
async def new_project(ctx): # memanggil fungsi saat perintah !new_project yang digunakan untuk menambahkan proyek baru
...
async def get_projects(ctx): # memanggil fungsi saat perintah !projects yang digunakan untuk menampilkan semua proyek
...
async def skills(ctx): # memanggil fungsi saat perintah !skills yang digunakan untuk menghubungkan keterampilan ke proyek
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:
...
async def delete_project(ctx): # memanggil fungsi saat perintah !delete yang digunakan untuk menghapus proyek
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]
...
async def update_projects(ctx): # memanggil fungsi saat perintah !update_projects yang digunakan untuk mengubah data proyek
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]
